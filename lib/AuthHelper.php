...
class AuthHelper
{
    ...
    /**
     * Verify if the request is made from shoplazza using hmac hash value
     *
     * @throws SdkException if SharedSecret is not provided or hmac is not found in the url parameters
     *
     * @return bool
     */
    public static function verifyShoplazzaRequest()
    {
        $data = $_GET;

        if(!isset(ShoplazzaSDK::$config['SharedSecret'])) {
            throw new SdkException("Please provide SharedSecret while configuring the SDK client.");
        }

        $sharedSecret = ShoplazzaSDK::$config['SharedSecret'];
     ...
     *
     * @see https://docs.shoplazza.com/api/guides/authentication/oauth#scopes For allowed scopes
     *
    ...
    public static function createAuthRequest($scopes, $redirectUrl = null, $state = null, $options = null, $return = false)
    {
        ...
        if (!$redirectUrl) {
            ...
            //If redirect url is the same as this url, then need to check for access token when redirected back from shoplazza
            if(isset($_GET['code'])) {
        ...
        // https://{shop}.myshoplazza.com/admin/oauth/authorize?client_id={api_key}&scope={scopes}&redirect_uri={redirect_uri}&state={nonce}&grant_options[]={option}
     ...
     * Call this when being redirected from shoplazza page ( to the $redirectUrl) after authentication
     *
    ...
    public static function getAccessToken()
    {
        ...
        if(self::verifyShoplazzaRequest()) {
            $data = array(
            ...
            throw new SdkException("This request is not initiated from a valid shoplazza shop!");
        }
    }
}
