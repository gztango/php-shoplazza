...
abstract class ShoplazzaResource
{
     ...
     *
     * Example : https://myshop.myshoplazza.com/admin/products
     *
     * @var string
     */
    ...
    /**
     * Create a new Shoplazza API resource instance.
     *
     * @param integer $id
     ...
     * @see: https://docs.shoplazza.com/en/api/guides/paginated-rest-results?utm_source=exacttarget&utm_medium=email&utm_campaign=api_deprecation_notice_1908
     * @var string $nextLink
     */
    ...
    public function __construct($id = null, $parentResourceUrl = '')
    {
        ...
        if (isset($config['ShoplazzaApiFeatures'])) {
            foreach($config['ShoplazzaApiFeatures'] as $apiFeature) {
                $this->httpHeaders['X-Shoplazza-Api-Features'] = $apiFeature;
            }
        }
    }

    /**
     * Return ShoplazzaResource instance for the child resource.
     *
     * @example $shoplazza->Product($productID)->Image->get(); //Here Product is the parent resource and Image is a child resource
    ...
    /**
     * Return ShoplazzaResource instance for the child resource or call a custom action for the resource
     *
     * @example $shoplazza->Product($productID)->Image($imageID)->get(); //Here Product is the parent resource and Image is a child resource
     * Called like an object method (with parenthesis) optionally with the resource ID as the first argument
     * @example $shoplazza->Discount($discountID)->enable(); //Calls custom action enable() on Discount resource
     *
     ...
     *
     * @return mixed / ShoplazzaResource
     */
     ...
     *
     * @param array $urlParams Check Shoplazza API reference of the specific resource for the list of URL parameters
     * @param string $url
     ...
     *
     * @param array $dataArray Check Shoplazza API reference of the specific resource for the list of required and optional data elements to be provided
     * @param string $url
     * @param bool $wrapData
     *
    ...
    public function getLink($responseHeaders, $type='next'){
        ...
        if(array_key_exists('x-shoplazza-api-version', $responseHeaders)
            && $responseHeaders['x-shoplazza-api-version'] < '2019-07'){
            return null;
        }
